...
from prompt_toolkit import completion, history, prompt
...
def quaternion_downconvert(q):
    ...
    if comp[1] != 0 or comp[2] != 0 or comp[3] != 0:
        return q
    else:
        return comp[0]
...
def p_experssion_func(t):
    ...
    if t[1] in functions:
        ...
        if t[3] is None:
            ...
            if type(t[3]) == list:
                pargs = [quaternion_downconvert(q) for q in t[3]]
            else:
                pargs = quaternion_downconvert(t[3])
            try:
...
def p_expression_binop(t):
    ...
    if t[2] == "+":
        t[0] = list_safe(t[1]) + list_safe(t[3])
    elif t[2] == "-":
        t[0] = list_safe(t[1]) - list_safe(t[3])
    elif t[2] == "*":
        t[0] = list_safe(t[1]) * list_safe(t[3])
